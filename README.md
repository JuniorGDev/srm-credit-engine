...
O projeto adota o padrão de commits convencionais para manter um histórico limpo e semântico:
...
- `chore:` - Atualizações de configuração/dependências

**Exemplos:**
```bash
git commit -m "feat: implement strategy pattern for receivable pricing"
git commit -m "fix: correct BigDecimal rounding in present value calculation"
git commit -m "test: add unit tests for DuplicataMercantilStrategy"
git commit -m "docs: update README with Docker instructions"
```

### Fluxo de Trabalho Baseado em Branches

1. **Branch Principal:** `main` - Código de produção estável
2. **Branches de Funcionalidade:** `feature/<nome-da-feature>` - Desenvolvimento isolado

**Processo:**
```bash
# Crie uma branch para sua funcionalidade
git checkout -b feature/calculation-discount

# Faça seus commits seguindo o Conventional Commits
git add .
...
# Abra um Pull Request para code review
# Após aprovação, mergear na main
```

**Regras:**
- Nunca commitar diretamente na `main`
- Sempre abrir Pull Request para revisão
- Manter as branches atualizadas com `main`
- Garantir que todos os testes passem antes do PR
